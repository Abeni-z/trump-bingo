...
import { useBingoStore, generateCard } from '../store/bingoStore.js'
...
export default function CardManager() {
  ...
  function handleBulkGenerate() {
    for (let i = 0; i < bulkCount; i++) {
      addCard(`Card ${cards.length + i + 1}`, generateCard())
    }
  }
  ...
  return (
    <div className="page">
      <div className="card" style={{marginBottom:16}}>
        <h2 style={{color:'var(--orange)', marginBottom:12, fontFamily: "'Inter', 'Roboto', sans-serif"}}>📋 {FULL_BRAND} — Card Manager</h2>
        <div style={{display:'flex', gap:8, flexWrap:'wrap', alignItems:'center'}}>
          <span style={{fontWeight:700, fontFamily: "'Inter', 'Roboto', sans-serif"}}>Bulk Generate:</span>
          <input type="number" min={1} max={200} value={bulkCount}
            onChange={e => setBulkCount(Number(e.target.value))}
            style={{width:70}} />
          <button className="btn btn-green" onClick={handleBulkGenerate}>✨ Generate {bulkCount} Cards</button>
      ...
      <div ref={printRef} style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(260px, 1fr))', gap:16}}>
        ...
        {cards.length === 0 && (
          <div style={{gridColumn:'1/-1', textAlign:'center', padding:40, color:'var(--text-muted)', fontFamily: "'Inter', 'Roboto', sans-serif"}}>
            No cards yet. Use Bulk Generate above to create cards.
          </div>
        )}
      </div>
